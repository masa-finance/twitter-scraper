...
// SearchTimeline — use x.com/i/api/graphql so URL path matches browser (x-client-transaction-id is path-sensitive).
const searchURL = "https://x.com/i/api/graphql/rkp6b4vtR9u7v3naGoOzUQ/SearchTimeline"
...
func (s *Scraper) getSearchTimeline(query string, maxNbr int, cursor string) (*searchTimeline, error) {
	...
	// Variables + features captured from live x.com SearchTimeline (Mar 2026).
